...
enum LUA_OS_CMD_TYPE {
    ...
    LUA_OS_CMD_TYPE_AYM_PLAY = 4,
};

enum LUA_OS_CMD_TYPE_AYM_PLAY {
    LUA_OS_CMD_TYPE_AYM_PLAY_CMD_TICK = 0
};
...
void aym_tick_event () {
    lua_os_cmd_t send_cmd_buf;
    send_cmd_buf.type = LUA_OS_CMD_TYPE_AYM_PLAY;
    send_cmd_buf.cmd = LUA_OS_CMD_TYPE_AYM_PLAY_CMD_TICK;
    xQueueSend(lua_os_cmd_queue, &send_cmd_buf, portMAX_DELAY);
}
