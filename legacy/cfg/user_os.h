...
/*
 * Длина очереди (в количестве элементов!), размер одного элемента в байтах,
 * указатель на uint8_t байт указатель на внутренную структуру самой очереди.
 */
#define USER_OS_STATIC_QUEUE_CREATE(QUEUE_LENGTH, ITEM_SIZE, P_BUF, P_OBJ_ST) \
        xQueueCreateStatic( QUEUE_LENGTH, ITEM_SIZE, P_BUF, P_OBJ_ST )
#define USER_OS_QUEUE_RECEIVE(QUEUE, P_BUF, TIMEOUT) \
        xQueueReceive(QUEUE,P_BUF,(TickType_t)TIMEOUT)
#define USER_OS_QUEUE_SEND(QUEUE, P_BUF, TIMEOUT) \
        xQueueSend(QUEUE,P_BUF,(TickType_t)TIMEOUT)
...
// Созданиче задач.
//**********************************************************************
#define USER_OS_STATIC_TASK_CREATE(FUNC, STRING_NAME, STACK_SIZE, PARAM, PRIO, P_STACK, P_STRUCT) \
        xTaskCreateStatic( FUNC, STRING_NAME, STACK_SIZE, PARAM, PRIO, P_STACK, P_STRUCT )
...
#define USER_OS_STATIC_TIMER_CREATE(STRING_NAME, PERIOD_MS, PARAM, FUNC, P_STRUCT) \
        xTimerCreateStatic(STRING_NAME,PERIOD_MS/portTICK_RATE_MS,pdTRUE,PARAM,FUNC,P_STRUCT)
