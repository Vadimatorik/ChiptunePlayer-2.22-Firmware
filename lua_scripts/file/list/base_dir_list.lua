...
function base_dir_list:new (path_to_dir, name, fat_fs_file_obj, open_flags)
    local o = {
...
function base_dir_list:open ()
    return open_file(self.path_to_dir, self.name, self.fat_fs_file_obj, self.open_flags)
end

function base_dir_list:close ()
    return close_file(self.path_to_dir, self.name, self.fat_fs_file_obj)
end

function base_dir_list:write_item (number, name)
    local lseek_byte = number * list_dir_item_len.sum_len
    local rv = lseek_file(path_to_dir, file_name, fat_fs_file_obj, lseek_byte)
    if rv ~= 0 then
        return rv
    end

    return write_file_string(path_to_dir, file_name, fat_fs_file_obj, name)
end
